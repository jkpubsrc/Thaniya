...
class _DataV1(AbstractCfgComponent):
	...
	__VALID_KEYS = [
		# NOTE: Convention: all data provided by clients that might be untrue are marked with "Client"
		...
		CfgKeyValueDefinition("nTotalBytesUploaded",	AmountOfBytes,	True,	AmountOfBytes.parse,	int		),	# This is the total number of bytes received from the client (including meta files) as measured by the server.
		CfgKeyValueDefinition("nTotalBytesArchived",	AmountOfBytes,	True,	AmountOfBytes.parse,	int		),	# This is the total number of bytes for this backup (including meta files) as measured by the server.
	...
	def __init__(self):
		...
		self._nTotalBytesUploaded = None		# AmountOfBytes
		self._nTotalBytesArchived = None		# AmountOfBytes
...
class _Magic(CfgComponent_Magic):
	...
	MAGIC = "thaniya-archived-backup-info"
	VERSION = 1
	VALID_VERSIONS = [ 1 ]
...
#
# Represents the contents of a backup information file within the backup archive.
#
class ArchivedBackupInfoFile(AbstractAppCfg):
	...
	def loadFromFile(filePath:str):
		...
		return ArchivedBackupInfoFile.loadFromJSON(jData)
	#
